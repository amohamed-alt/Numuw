...
class AiCareEventDraft {
  ...
  factory AiCareEventDraft.fromJson(Map<String, dynamic> map) {
    final eventType = _normalizeEventType(map['event_type'] ?? map['eventType']);
    if (eventType == null) {
      throw const FormatException('Unsupported or missing event type.');
    }

    final startedAt = _dateTimeFromAny(map['started_at'] ?? map['startedAt'] ?? map['time']);
    final endAt = _dateTimeFromAny(map['ended_at'] ?? map['endedAt']);
    final feedingMethods = _normalizeMethods(map['feeding_methods'] ?? map['feedingMethods'] ?? map['feeding_method']);
    ...
    final left = _double(map['left_amount_ml'] ?? map['leftAmountMl'] ?? map['left_pumped_ml'] ?? map['leftPumpedAmountMl']);
    final right = _double(map['right_amount_ml'] ?? map['rightAmountMl'] ?? map['right_pumped_ml'] ?? map['rightPumpedAmountMl']);
    ...
    final needsReview = _bool(map['needs_review'] ?? map['needsReview']) ?? true;
    final timeNeedsReview = _bool(map['time_needs_review'] ?? map['timeNeedsReview']) ?? false;
    final dateNeedsReview = _bool(map['date_needs_review'] ?? map['dateNeedsReview']) ?? false;
  ...
  Map<String, dynamic> toSaveArguments() {
    final metadata = <String, dynamic>{
      ...
      if (eventType == 'pumping' && (pumpingLeftAmountMl != null || pumpingRightAmountMl != null)) 'quantity_mode': 'split',
      if (eventType == 'feeding' && feedingMethods.isNotEmpty) 'feeding_methods': feedingMethods,
...
class AiAssistantResponse {
  ...
  factory AiAssistantResponse.fromJson(Map<String, dynamic> map) {
    final sections = (map['sections'] is List ? map['sections'] as List : const [])
        .whereType<Map>()
        .map((item) => AiSummarySection.fromJson(Map<String, dynamic>.from(item)))
        .toList(growable: false);
    final actions = (map['actions'] is List ? map['actions'] as List : const [])
        .whereType<Map>()
        .map((item) => AiCareEventDraft.fromJson(Map<String, dynamic>.from(item)))
        .toList(growable: false);
...
String? _normalizeEventType(Object? value) {
  ...
  const allowed = <String>{'feeding', 'sleep', 'diaper', 'food', 'medicine', 'temperature', 'note', 'pumping'};
...
List<String> _normalizeMethods(Object? value) {
  ...
  return values.map(_string).whereType<String>().map((item) => item.toLowerCase()).where(allowed.contains).toSet().toList(growable: false);
}
