          ...
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('اسألي نُمُوّ', style: TextStyle(color: numuwTextColor(), fontSize: 21, fontWeight: FontWeight.w900)),
                        Text('مساعدة يومية من تسجيلات طفلك', style: TextStyle(color: numuwSecondaryTextColor(), fontSize: 12.5)),
                      ],
                    ),
                  ),
                  AppIconButton(
                    icon: Icons.add_comment_outlined,
                    onPressed: () => setState(() { _messages.clear(); _error = null; }),
                    badge: false,
                    size: 44,
                    radius: 14,
                  ),
                  ...
                  if (_messages.isEmpty) ...[
                    _Card(
                      child: Text(
                        ...
                        style: TextStyle(color: numuwTextColor(), fontSize: 15, height: 1.75, fontWeight: FontWeight.w600),
                      ),
                    ),
                    const SizedBox(height: 12),
                    Container(
                      padding: const EdgeInsetsDirectional.all(13),
                      decoration: BoxDecoration(color: AppColors.blueLight, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.blue.withValues(alpha: .24))),
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const Icon(Icons.info_outline_rounded, color: AppColors.blue, size: 18),
                        const SizedBox(width: 9),
                        Expanded(child: Text('لا ترسلي معلومات شخصية غير ضرورية أو صورًا حساسة.', style: TextStyle(color: numuwTextColor(), fontSize: 13, height: 1.55))),
                      ]),
                    ),
                    const SizedBox(height: 18),
                    Text('أسئلة مقترحة', style: TextStyle(color: numuwSecondaryTextColor(), fontSize: 13, fontWeight: FontWeight.w700)),
                    const SizedBox(height: 10),
                    ...suggestions.map((text) => _Suggestion(text: text, onTap: child == null ? null : () => _send(text))),
                  ] else ...[
                    ..._messages.map((m) => _Bubble(message: m)),
                    if (_loading)
                      Align(alignment: AlignmentDirectional.centerStart, child: _Card(child: const LoadingDots())),
                  ],
                  if (_error != null) ...[
                    const SizedBox(height: 12),
            ...
            Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: numuwSurfaceColor(), borderRadius: BorderRadius.circular(20), border: Border.all(color: numuwBorderColor())),
                child: Row(children: [
                  Icon(Icons.attach_file_rounded, color: numuwSecondaryTextColor(), size: 20),
                  const SizedBox(width: 8),
                  Expanded(child: TextField(controller: _input, enabled: child != null && !_loading, minLines: 1, maxLines: 4, textDirection: TextDirection.rtl, decoration: const InputDecoration(hintText: 'اكتبي سؤالك...', border: InputBorder.none, filled: false, isDense: true), onSubmitted: (_) => _send())),
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: _loading ? const Padding(padding: EdgeInsets.all(13), child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.send_rounded, color: Colors.white, size: 21),
                    ),
                  ),
                ]),
              ),
...
class _Suggestion extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Material(color: numuwSurfaceColor(), borderRadius: BorderRadius.circular(16), child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16), child: Container(padding: const EdgeInsetsDirectional.all(14), decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: numuwBorderColor())), child: Row(children: [Expanded(child: Text(text, style: TextStyle(color: numuwTextColor(), fontSize: 14, height: 1.55, fontWeight: FontWeight.w600))), Icon(Icons.chevron_left_rounded, color: numuwSecondaryTextColor())])))),
  );
}
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: user ? numuwAccentColor() : numuwSurfaceColor(), borderRadius: BorderRadius.circular(18), border: user ? null : Border.all(color: numuwBorderColor())),
        child: Text(message.text, style: TextStyle(color: user ? Colors.white : numuwTextColor(), fontSize: 14, height: 1.65, fontWeight: FontWeight.w600)),
      ),
    );
  }
}
